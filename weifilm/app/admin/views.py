#coding:utf8
from . import admin
from app.admin.forms import RoleForm,LoginForm, TagForm, MovieForm,PreviewForm,PwdForm,AuthForm,AdminForm
from flask import render_template, redirect, url_for, flash, session, request,abort
from app.models import Role,Auth,Admin, Tag, Movie,Preview,User,Comment,Moviecol,Adminlog,Userlog,Oplog
...
def movie_add():
    ...
    if form.validate_on_submit():
        ...
        form.logo.data.save(app.config['UP_DIR'] + logo+'.png')
        print()
        ...
        print(app.config['UP_DIR'])
        print(logo+'.png')
        print(app.config['UP_DIR'] + logo + '.png')
        movie = Movie(
            title=data['title'],
            url=url,
            info=data['info'],
...
def movie_edit(id):
    ...
    if form.validate_on_submit():
        ...
        os.remove(app.config['UP_DIR']+movie.logo+'.png')
        file_url = secure_filename(form.url.data.filename)
        url = change_filename(file_url)
        form.url.data.save(app.config['UP_DIR'] + url)
        file_logo = secure_filename(form.logo.data.filename)
...
def preview_list(page=None):
    if page is None:
        page = 1
    page_data = Preview.query.filter_by(
    ).order_by(
...
def preview_edit(id):
    ...
    if form.validate_on_submit():
        ...
        if not os.path.exists(app.config['UP_DIR']):
            os.makedirs(app.config['UP_DIR'])
            os.chmod(app.config['UP_DIR'], "rw")
        os.remove(app.config['UP_DIR'] + preview.logo + '.png')
        logo = change_filename(file_logo)
        form.logo.data.save(app.config['UP_DIR']+logo+'.png')
        preview.logo=logo,
...
def admin_edit(id):
    ...
    return render_template("admin/admin_edit.html", form=form,auth=admin)
